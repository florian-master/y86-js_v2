...
import {Instruction, InstructionSet} from "../../../model/instructionSet";
...
test("halt with changed icode test", () => {

    let kernelController = new KernelController('seq')
    let sim = kernelController.getSim() as Sim;
    sim.memory.loadProgram(prog);

    // change instr set
    kernelController.getInstructionSet().getHandle().set("halt", new Instruction("halt", 1, 0, []));
    kernelController.getInstructionSet().getHandle().set("nop", new Instruction("nop", 0, 0, []));
    ...
    // try again with new icode for halt
    kernelController = new KernelController('seq')
    sim = new KernelController('seq').getSim() as Sim;
    sim.memory.loadProgram(prog2);

    // change instr set
    kernelController.getInstructionSet().getHandle().set("halt", new Instruction("halt", 0, 0, []));
    kernelController.getInstructionSet().getHandle().set("nop", new Instruction("nop", 1, 0, []));
